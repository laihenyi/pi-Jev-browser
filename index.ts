...
const actionSchema = strictObject({
	...
	deltaX: Type.Optional(Type.Number({ description: 'Horizontal wheel distance in CSS pixels for "scroll"; required with deltaY. Optional x/y set the pointer position first.' })),
	deltaY: Type.Optional(Type.Number({ description: 'Vertical wheel distance in CSS pixels for "scroll"; positive scrolls down.' })),
	text: Type.Optional(Type.String({ description: 'Characters to type into the focused element for "type". Use fill with a target when the field is addressable.' })),
	ms: Type.Optional(Type.Number({ minimum: 0, maximum: 30000, description: 'Pause length for "wait"; omit to wait for the page to settle.' })),
	keys: Type.Optional(Type.Array(Type.String(), { description: 'Key names for "keypress" (for example "Enter", "Control+a"), or modifier keys held during "click".' })),
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Jev Run",
		description:
			"Start or reuse an isolated Playwright browser, capture before/after screenshots, and let Jev (TypeSafe System One) advance a narrowly scoped browser goal in a bounded DOM loop. Requires TYPESAFE_API_KEY from the process environment or typesafe.apiKey in pi-jev-browser.config.json. Field text for typed inputs is generated by the active pi model. Stops on REVIEW, BLOCKED, no progress, an error, or the step limit. Returns the run status (done_unverified is Jev's claim, not proof), the steps, elapsedMs, tracePath, the final page text, and the final screenshot (finalScreenshotPath when the image is not displayed). Page text and field values are sent to TypeSafe and to the active pi model.",
		promptSnippet:
		...
		promptGuidelines: [
			...
			"The user only needs to provide a URL and a goal for jev_run; do not ask them to specify the internal workflow.",
		...
		parameters: strictObject({
			kind: Type.Optional(
				StringEnum(["text", "table", "links", "attributes"] as const, {
					description:
						"What to read: visible text (default), the rows of the first matching table, anchor text with href, or one attribute of each matching element.",
				}),
			),
	...
	pi.registerTool({
		...
		parameters: strictObject({
			...
			limit: Type.Optional(
				Type.Number({ minimum: 1, maximum: 1000, description: "Maximum number of log entries to return." }),
			),
		}),
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			return withStatus(ctx, signal, async (host) => {
		...
		parameters: strictObject({
			...
			intervalMs: Type.Optional(
				Type.Number({
					minimum: 250,
					maximum: 10000,
					description: 'Screenshot refresh interval for "start". Defaults to the configured stream.intervalMs.',
				}),
			),
		}),
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
